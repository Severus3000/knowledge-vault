...
source: "https://mp.weixin.qq.com/s/vS3ffwm7L-9CzedhMIcY1g"
platform: "wechat"
author: "阿里云开发者"
date: 2026-03-26
ingested: 2026-04-04T15:55:00Z
tags: [openclaw, agent-framework, self-iteration, memory, cron, heartbeat, agent-eval, a2a]
...
- 风险预警 Agent — 客户行业风险、经营风险、产品风险、合规风险（新增）
...
...
两者区别可查看：https://docs.openclaw.ai/automation/cron-vs-heartbeat#cron-vs-heartbeat
...
| 响应处理 | HEARTBEAT_OK跳过、去重、截取 | 标准agent响应 |
...
```
1. 检查配置          → agents/<agent-id>/config.yaml
       ↓
2. 读取规则          → agents/<agent-id>/eval_rule.md
       ↓
3. 生成用例          → user_cases/<agent-id>/UC-XXX.md
       ↓
4. 创建 todo         → user_cases/<agent-id>/UC-XXX-todo.md
       ↓
5. 执行测试          → 围绕 todo.md 勾选执行
       ↓
6. 发送反馈          → 工作反馈格式（不透露测试）
       ↓
7. 生成 JSON         → results/UC-XXX-result.json
```
...
| 维度 | OpenClaw (二级存储 + QMD 检索模式) | OpenViking |
|------|-------------------------------------|------------|
| 核心存储范式 | Markdown 文件是唯一真相来源，所有记忆直接写成 .md 文件 | 文件系统范式（虚拟文件系统 viking://），内容分层组织（L0/L1/L2） |
| 内容存储层 | 本地文件系统（MEMORY.md + memory/**/*.md），纯 Markdown | AGFS（内容存储层），支持 S3 / 本地 / memory 等后端 |
| 索引/检索层 | 可插拔：默认 SQLite + sqlite-vec + FTS5；QMD 模式为实验后端（BM25 + 向量 + LLM reranking） | 独立向量库（只存 URI、向量、元数据，不存内容） |
| 二级存储体现 | Markdown 文件即"磁盘"（持久层），检索后端作为"二级索引" | 明确双层分离：AGFS（内容） + 向量库（索引） |
| 分层/压缩机制 | 社区常见 2-3 层（Daily notes → 提取到 MEMORY.md），靠 compaction / nightly job | 内置 L0/L1/L2 三级上下文，按需加载 |
| 检索方式 | QMD 模式：BM25 + 向量 + reranking 混合 | 目录递归检索 + 语义搜索，支持路径定位 + 向量 |
| 分块粒度 | ~400 token 块 + 80 token overlap | L0/L1/L2 不同粒度，整目录/文件递归加载 |
| 自我迭代/进化 | 依赖 Agent 主动写文件 + compaction；无内置循环 | 内置 memory self-iteration loops，会话结束可触发异步记忆提取与更新 |
| 典型适用场景 | 本地优先、极致透明、可读性强、Markdown 爱好者 | 需要严格省 token、上下文分层管理、大规模资源统一管理 |
| 当前成熟度 | QMD 为实验功能，默认仍为 SQLite | 字节开源，设计较新但架构完整 |
